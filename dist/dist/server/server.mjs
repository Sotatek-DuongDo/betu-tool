...
import require$$0, { getCurrentInstance, reactive, isRef, ref, toRef, defineComponent, computed, h, resolveComponent, watchEffect, markRaw, inject, provide, Suspense, Transition, toRaw, useSSRContext, onMounted, withCtx, createTextVNode, unref, createVNode, openBlock, createBlock, createCommentVNode, Fragment as Fragment$1, shallowRef, mergeProps, toDisplayString as toDisplayString$1, onErrorCaptured, createApp } from "vue";
import { withBase, withQuery, hasProtocol, isEqual } from "ufo";
...
const components = {};
function componentsPlugin_49e4bd44(nuxtApp) {
...
const vueuseHead_fa522846 = defineNuxtPlugin((nuxtApp) => {
...
const _6711fdd1 = defineNuxtPlugin((nuxtApp) => {
...
const authAdmin = async (username, password) => {
  const result = await login$1(username, password);
  adminToken.value = result.access_token;
  console.log("res", result);
};
const getAvailableBetu$1 = async (userToken) => {
  const res = await request({
...
const joinRound$1 = async (userToken, balance) => {
  const res = await request({
...
const getCurrentRound$1 = async (userToken) => {
  const res = await request({
...
const getMyBets$1 = async (userToken) => {
  const res = await request({
...
const placeBet = async (userToken, data) => {
  const res = await request({
...
const placeMultiBet = async (userToken, data) => {
  const res = await request({
...
const getFixtureDetail = async (fixtureId) => {
  const res = await request({
...
const useHttp = () => {
  return {
    ...
    createUser,
    isAuth,
    getAvailableBetu: getAvailableBetu$1,
    getCurrentRound: getCurrentRound$1,
    joinRound: joinRound$1,
    getConfigs: getConfigs$1,
    getMyBets: getMyBets$1,
    placeBet,
    placeMultiBet,
    getFixtureDetail
  };
  ...
  actions: {
    async login(username, password) {
      ...
      const newUser = {
        ...
        betuHold: (_a = currentRound.joinedInfo) == null ? void 0 : _a.betuHold,
        isJoined: !!currentRound.joinedInfo,
        joinRound: async () => {
          const self2 = _.find(this.users, { accessToken });
          if (self2.isJoined)
            return;
          const maxStake = this.configs["MAX_STAKE"];
          const balance2 = new BigNumber(self2.availableBalance).gt(maxStake || 0) ? maxStake : self2.availableBalance;
          const joined = await joinRound(self2.accessToken, balance2);
          self2.betuHold = joined.betuLock;
        },
    ...
    async getConfigs() {
      ...
      this.configs = _.chain(result).map((item) => [item.key, item.value]).fromPairs().value();
    }
  },
  persist: {
    key: "test",
...
const _sfc_main$7 = /* @__PURE__ */ defineComponent({
  ...
  setup(__props) {
    const { authAdmin: authAdmin2, placeMultiBet: placeMultiBet2, placeBet: placeBet2, getFixtureDetail: getFixtureDetail2 } = useHttp();
    ...
    const gridData = ref([]);
    const jsonData = ref();
    const userOptions = computed(() => userStore.users.map((item) => ({
      value: item.username,
      label: item.username
    })));
    ...
    const handleAddUser = async () => {
      try {
        await userStore.login(formLoginData.value.username, formLoginData.value.password);
    ...
    const handleBet = async () => {
      ...
      for (const item of jsonData.value.data) {
        promises[item.fixture_id] = getFixtureDetail2(item.fixture_id).then((data) => {
          const temps = _.get(data, "group.templates");
          outcomes = [
            ...outcomes,
            ..._.chain(temps).map("markets").flatten().map("outcomeGroups").flatten().map("outcomes").flatten().value()
          ];
        });
      }
      for (const username of userSelected.value) {
        ...
        try {
          if (isMultiBet) {
            await Promise.all(_.map(jsonData.value.data, async (item) => {
              var _a;
              await promises[item.fixture_id];
              const currentOdds = (_a = _.find(outcomes, [
                "id",
                item.bet_game_id
              ])) == null ? void 0 : _a.currentOdd;
              if (currentOdds)
                item.odds = currentOdds;
            }));
            await placeMultiBet2(user.accessToken, jsonData.value);
          } else {
            await placeBet2(user.accessToken, jsonData.value);
          }
        } catch (e) {
          console.log(e);
        }
    ...
    onMounted(async () => {
      await authAdmin2("admin", "Admin_12345");
    ...
    return (_ctx, _push, _parent, _attrs) => {
      ...
      const _component_json_viewer = resolveComponent("json-viewer");
      const _component_el_form = resolveComponent("el-form");
      const _component_el_form_item = resolveComponent("el-form-item");
      const _component_el_input = resolveComponent("el-input");
      _push(`<!--[--><div><div style="${ssrRenderStyle({ "margin-bottom": "10px" })}">`);
      ...
      _push(ssrRenderComponent(_component_el_button, {
        type: "primary",
        onClick: () => userFileRef.value.click()
      }, {
        default: withCtx((_2, _push2, _parent2, _scopeId) => {
          if (_push2) {
            _push2(`Import users`);
            ...
            return [
              createTextVNode("Import users")
            ];
          }
        }),
        _: 1
      }, _parent));
      _push(`<input type="file" style="${ssrRenderStyle({ "display": "none" })}"></div>`);
        ...
        default: withCtx((_2, _push2, _parent2, _scopeId) => {
          if (_push2) {
            ...
            _push2(ssrRenderComponent(_component_el_table_column, null, {
              header: withCtx((_3, _push3, _parent3, _scopeId2) => {
                if (_push3) {
              ...
              default: withCtx(({ row }, _push3, _parent3, _scopeId2) => {
                if (_push3) {
                  ...
                  return [
                    row.loading !== false ? (openBlock(), createBlock("div", { key: 0 }, "Loading...")) : createCommentVNode("", true),
                    row.loading === false ? (openBlock(), createBlock(Fragment$1, { key: 1 }, [
                      createVNode(_component_el_button, {
                        type: "primary",
                        size: "small",
                        onClick: row.joinRound
      ...
      _push(ssrRenderComponent(_component_el_dialog, {
        ...
        default: withCtx((_2, _push2, _parent2, _scopeId) => {
          if (_push2) {
            _push2(ssrRenderComponent(_component_el_table_v2, {
              data: gridData.value,
              columns,
              width: 900,
              height: 400
      ...
      _push(ssrRenderComponent(_component_el_dialog, {
        ...
        default: withCtx((_2, _push2, _parent2, _scopeId) => {
          if (_push2) {
            ...
            _push2(ssrRenderComponent(_component_el_select_v2, {
              modelValue: userSelected.value,
              "onUpdate:modelValue": ($event) => userSelected.value = $event,
              options: unref(userOptions),
              style: { "width": "240px" },
              multiple: "",
              "collapse-tags": "",
              "collapse-tags-tooltip": ""
            }, null, _parent2, _scopeId));
            _push2(ssrRenderComponent(_component_json_viewer, { value: jsonData.value }, null, _parent2, _scopeId));
            _push2(ssrRenderComponent(_component_el_button, { onClick: handleBet }, {
              default: withCtx((_3, _push3, _parent3, _scopeId2) => {
                if (_push3) {
                  _push3(`Confirm bet`);
                } else {
                  return [
                    createTextVNode("Confirm bet")
                  ];
                }
              }),
              _: 1
            ...
            return [
              ...
              createVNode(_component_json_viewer, { value: jsonData.value }, null, 8, ["value"]),
              createVNode(_component_el_button, { onClick: handleBet }, {
                default: withCtx(() => [
                  createTextVNode("Confirm bet")
                ]),
                _: 1
      ...
      _push(ssrRenderComponent(_component_el_dialog, {
        modelValue: dialogLoginVisible.value,
        "onUpdate:modelValue": ($event) => dialogLoginVisible.value = $event,
        title: "Add user",
        width: "400px"
      }, {
        footer: withCtx((_2, _push2, _parent2, _scopeId) => {
          if (_push2) {
            ...
            _push2(ssrRenderComponent(_component_el_button, {
              onClick: ($event) => dialogLoginVisible.value = false
            }, {
              default: withCtx((_3, _push3, _parent3, _scopeId2) => {
                if (_push3) {
                  _push3(`Cancel`);
        ...
        default: withCtx((_2, _push2, _parent2, _scopeId) => {
          if (_push2) {
            _push2(ssrRenderComponent(_component_el_form, {
              model: formLoginData.value,
              "label-width": "150px",
              "label-position": "top"
            }, {
              default: withCtx((_3, _push3, _parent3, _scopeId2) => {
                if (_push3) {
                  _push3(ssrRenderComponent(_component_el_form_item, { label: "Username" }, {
                    default: withCtx((_4, _push4, _parent4, _scopeId3) => {
                      if (_push4) {
                        _push4(ssrRenderComponent(_component_el_input, {
                          modelValue: formLoginData.value.username,
                          "onUpdate:modelValue": ($event) => formLoginData.value.username = $event
                  ...
                  _push3(ssrRenderComponent(_component_el_form_item, { label: "Password" }, {
                    default: withCtx((_4, _push4, _parent4, _scopeId3) => {
                      if (_push4) {
                        _push4(ssrRenderComponent(_component_el_input, {
                          modelValue: formLoginData.value.password,
                          "onUpdate:modelValue": ($event) => formLoginData.value.password = $event,
                          type: "password"
                        }, null, _parent4, _scopeId3));
                      } else {
                        return [
...
const routes = [
  {
    name: "index",
    path: "/",
    file: "/home/dzuongdo/bet_u/betu-tool/pages/index.vue",
    children: [],
    meta,
...
const _4196fd93 = defineNuxtPlugin(async (nuxtApp) => {
...
const index$1 = "";
const elementUi_d4bb2082 = defineNuxtPlugin((nuxtApp) => {
  nuxtApp.vueApp.use(ElementPlus);
  nuxtApp.provide("loading", ElLoadingService);
});
const pinia_897f62ee = defineNuxtPlugin((nuxtApp) => {
  nuxtApp.$pinia.use(createNuxtPersistedState(useCookie));
});
const vue3Components_59fa1ac0 = defineNuxtPlugin((nuxtApp) => {
  nuxtApp.vueApp.use(JsonViewer);
});
const _plugins = [
  preload,
  componentsPlugin_49e4bd44,
  vueuseHead_fa522846,
  _6711fdd1,
  _4196fd93,
  elementUi_d4bb2082,
  pinia_897f62ee,
  vue3Components_59fa1ac0
];
